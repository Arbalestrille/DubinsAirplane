...
def main():
    ...
    if dubins_case == 1:  # short climb RSR
        ...
        start_node = np.array(
            [0, 0, -100, 0 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array(
            [0, 200, -125, 270 * pi / 180, VehiclePars.Vairspeed_0])
    elif dubins_case == 2:  # short climb RSL
        print('Path Type: short climb RSL')
        start_node = np.array(
            [0, 0, -100, -70 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array(
            [100, 100, -125, -70 * pi / 180, VehiclePars.Vairspeed_0])
        ...
        start_node = np.array(
            [0, 0, -100, 0 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array([100, -100, -200, -90 * pi /
                             180, VehiclePars.Vairspeed_0])
    elif dubins_case == 13:  # intermediate climb RSLR (descend at end)
        print('Path Type: intermediate climb RSLR (descend at end)')
        start_node = np.array(
            [0, 0, -200, 0 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array(
            [100, 100, -100, 90 * pi / 180, VehiclePars.Vairspeed_0])
        ...
        start_node = np.array(
            [0, 0, -200, 70 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array(
            [100, -100, -100, 90 * pi / 180, VehiclePars.Vairspeed_0])
    elif dubins_case == 16:  # intermediate climb LSRL (descend at end)
        print('Path Type: intermediate climb LSRL (descend at end)')
        start_node = np.array(
            [0, 0, -150, 0 * pi / 180, VehiclePars.Vairspeed_0])
        end_node = np.array([100, -100, -100, -90 * pi /
                             180, VehiclePars.Vairspeed_0])
    elif dubins_case == 0:  # for fixing errors
        print('Path Type: for fixing errors')
        start_node = np.array([0, 0, 0, 0, VehiclePars.Vairspeed_0])
        end_node = np.array([40, -140, 100, 11 * pi / 9,
                             VehiclePars.Vairspeed_0])  # LSRL
        end_node = np.array([60, 140, -140, 0 * pi / 14,
                             VehiclePars.Vairspeed_0])  # RLSL
        end_node = np.array([40, -140, -100, 10 * pi / 180,
                             VehiclePars.Vairspeed_0])  # LRSR
    ...
    if flag_nc == 0:
        ...
        R_min = MinTurnRadius_DubinsAirplane(
            VehiclePars.Vairspeed_0, VehiclePars.Bank_max)
        ...
        # dubins path.
        if (np.linalg.norm(end_node[0:2] -
                           start_node[0:2], ord=2) < 6 * R_min):
            print(
                "Conservative condition (end_node[0:2] - start_node[0:2],ord=2) < 6*R_min) not fulfilled!")
            print(
                "Start and end pose are close together. Path of type RLR, LRL may be optimal")
            print("May fail to compute optimal path! Aborting")
            sys.exit()

        DubinsAirplaneSolution = DubinsAirplanePath(
            start_node, end_node, R_min, VehiclePars.Gamma_max)

        path_dubins_airplane = ExtractDubinsAirplanePath(
        ...
        if ExFlags.plot:
            ...
            plot3(path_dubins_airplane[:,
                                       0],
                  path_dubins_airplane[:,
                                       1],
                  path_dubins_airplane[:,
                                       2],
                  'o',
